...
public class Money implements Comparable {
	...
	public String toString() {
		// Chia cho 100 để lấy phần thập phân
			double formattedAmount = (double) amount / 100;
			return formattedAmount + " " + currency.getName();
	}
	...
	public boolean equals(Money other) {
		// Thực hiện so sánh dựa trên giá trị UniversalValue (giá trị chung), để đảm bảo tính chính xác khi so sánh giữa các đơn vị tiền tệ khác nhau (SEK vs EUR).
	...
	public Money add(Money other) {
		// Đổi tiền của đối tượng kia sang đơn vị của mình trước khi cộng
		Integer otherAmountInThisCurrency = this.currency.valueInThisCurrency(other.getAmount(), other.getCurrency());
	...
	public Money sub(Money other) {
		// Đổi tiền của đối tượng kia sang đơn vị của mình trước khi trừ
		Integer otherAmountInThisCurrency = this.currency.valueInThisCurrency(other.getAmount(), other.getCurrency());
		return new Money(this.amount - otherAmountInThisCurrency, this.currency);
	}
	...
	public Money exchangeTo(Currency targetCurrency) {
		// Chuyển giá trị của mình sang đơn vị tiền tệ mục tiêu
		Integer newAmount = targetCurrency.valueInThisCurrency(this.amount, this.currency);
